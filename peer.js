...
function getConfig() {
  if (customIceServers && customIceServers.length > 0) {
    return { iceServers: customIceServers };
  }
...
export function setOnMessage(cb) { onMessage = cb; }
export function setOnConnectionChange(cb) { onConnectionChange = cb; }
export function isConnected() { return connections.size > 0; }
export function connectionCount() { return connections.size; }
export function hasConnection(connId) { return connections.has(connId); }
...
function setupDataChannel(connId, dc) {
  ...
  dc.onopen = () => {
    ...
    if (queue && queue.length > 0) {
      dc.send(JSON.stringify({ type: "_ice", data: queue }));
...
function onIceCandidate(connId, e) {
  ...
  if (conn && conn.dc && conn.dc.readyState === "open") {
    conn.dc.send(JSON.stringify({ type: "_ice", data: [e.candidate] }));
  } else {
...
function decodeSignal(blob) {
  ...
  return { type: type === "o" ? "offer" : "answer", connId, sdp: sdpB64 };
}
...
// --- Host: create offer ---
export async function createOffer(userId, displayName, setId) {
  ...
  connections.set(connId, { pc, dc, setId });
  ...
  return {
    ...
    code: JSON.stringify({
      ...
      user_id: userId, display_name: displayName, set_id: setId,
    }),
...
// --- Guest: accept offer ---
export async function acceptOffer(blob, userId, displayName) {
  const signal = decodeSignal(blob);
  const connId = signal.connId || String(++connCounter);

  const pc = new RTCPeerConnection(getConfig());
  pc.onicecandidate = (e) => onIceCandidate(connId, e);
  pc.ondatachannel = (e) => { setupDataChannel(connId, e.channel); };
  ...
  connections.set(connId, { pc, dc: null, setId: signal.set_id || signal.setId });
...
// --- Finalize (host pastes guest's answer) ---
export async function finalizeConnection(connId, blob) {
  const signal = decodeSignal(blob);
  const conn = connections.get(connId);
  if (!conn) return;
...
export function send(msg, connId = null) {
