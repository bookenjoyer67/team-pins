import { A as generate_dek, B as verify, D as encrypt_pin_data, E as encrypt_geojson, F as generate_uuid, I as serialize_container, L as sign, O as encrypt_raw_bytes, S as deserialize_container, T as encrypt_bytes_with_password, V as wrap_dek, _ as decrypt_bytes_with_password, a as base64_encode, b as decrypt_raw_bytes, c as compact_and_pack_json, d as compress_gzip_max, h as decompress_gzip, i as base64_decode, k as encrypt_with_password, l as compact_pack_gzip_json, m as decode_hex, s as base64url_encode, u as compress_gzip, v as decrypt_geojson, w as encode_hex, x as decrypt_with_password, y as decrypt_pin_data, z as unwrap_dek } from "./e2e_core2.js";
import { $ as saveSchema, B as importPin, C as getCommunity, G as saveChain, I as getTeam, J as saveDrawing, M as getSchemas, N as getSettings, O as getPin, R as importDrawing, U as saveAnnotation, V as importPins, W as saveAnnotations, X as saveLayers, Y as saveKnownPeer, Z as savePin, _ as getAnnotationsByCommunity, a as deleteDrawing, b as getChainsByCommunity, et as saveSettings, f as getAllDrawings, g as getAnnotation, it as saveTombstone, p as getAllPins, q as saveCommunity, r as deleteChain, rt as saveTeam, s as deletePin, v as getAnnotationsByPin, w as getDrawing, z as importDrawings } from "./db2.js";
import { a as isConnected, c as setOnConnectionChange, i as hasConnection, l as setOnMessage, n as createOffer, o as send, r as finalizeConnection, t as acceptOffer } from "./peer2.js";
import { t as state } from "./state.js";
import { a as promptRoomPassword, d as showQRHostDialog, f as showQRJoinDialog, l as showProgressDialog, m as toast, n as confirmDialog, r as escapeHtml, s as showPasswordDialog, t as alertDialog } from "./dialogs.js";
import { n as DeferredChunkStore, t as DeferredBoundedMap } from "./store-helpers.js";
//#region sync.js
...
var MAX_CHUNKS = 500;
var MAX_BATCH_CHUNKS = 200;
var chunkStore = new DeferredChunkStore(MAX_CHUNKS, 6e4);
var syncBatchStore = new DeferredBoundedMap(200, 3e4);
window._peerLocations = new DeferredBoundedMap(200, 12e4);
function splitMessage(msg) {
	if (msg.length < 16e3) return null;
	const id = crypto.randomUUID();
	const nonce = crypto.randomUUID();
	const total = Math.ceil(msg.length / 15e3);
	const chunks = [];
	for (let i = 0; i < total; i++) chunks.push(msg.slice(i * 15e3, (i + 1) * 15e3));
	return {
		id,
		nonce,
		total,
...
function accumulateBatch(key, batchIndex, totalBatches, data) {
	...
	if (!entry) {
		entry = {
			chunks: new Array(totalBatches),
			count: 0
		};
		syncBatchStore.set(key, entry);
	}
	if (entry.chunks[batchIndex] === void 0) {
...
function stripEmpties(obj, depth = 0) {
	...
	if (obj && typeof obj === "object") {
		...
		for (const [k, v] of Object.entries(obj)) {
			const c = stripEmpties(v, depth + 1);
			if (c !== void 0 && c !== null && c !== "" && !(Array.isArray(c) && c.length === 0) && !(c && typeof c === "object" && !Array.isArray(c) && Object.keys(c).length === 0)) out[k] = c;
		}
		return out;
	}
	return obj;
}
var HEX_KEYS = new Set([
	"public_key",
	"secret_key",
	"wrapped_dek",
	"ciphertext",
...
function walkHexFields(obj, fn, depth = 0) {
	...
	if (obj && typeof obj === "object") {
		const out = {};
		for (const [k, v] of Object.entries(obj)) out[k] = HEX_KEYS.has(k) && typeof v === "string" ? fn(v) : walkHexFields(v, fn, depth + 1);
		return out;
	}
	return obj;
}
...
function unpackHexFields(data) {
	return walkHexFields(data, (v) => {
		...
		try {
			return encode_hex(base64ToBytes(v));
		} catch (_) {
			return v;
		}
	});
...
async function compactStoredMedia(data, onProgress, compressVideos = false) {
	...
	const total = [...data.pins || [], ...data.drawings || []].filter((i) => i.media).length;
	let done = 0;
	let savedBytes = 0;
	...
	const compact = async (item) => {
		...
		try {
			...
			if (m.type && m.type.startsWith("video/") && compressVideos) {
				...
				if (result.compressed === false) toast("Video skipped — " + (result.reason || "unsupported format"), "#f97316");
			} else if (m.type && m.type.startsWith("image/") && !m.type.includes("gif") && !m.type.includes("svg")) result = await compressImageBuffer(raw, m.type, m.name);
	...
	if (total > 0) if (savedBytes > 0) {
		const mb = (savedBytes / 1024 / 1024).toFixed(1);
		toast(`Compacted ${compressedCount} media item${compressedCount !== 1 ? "s" : ""} (${mb} MB saved)`, "#16a34a");
	} else toast("Media already optimal — no savings", "#6b7280");
	return data;
}
var _meshBroadcast = null;
var passwordAuthPeers = /* @__PURE__ */ new Set();
function setMeshBroadcast(cb) {
	_meshBroadcast = cb;
}
function setupPeer() {
	setOnMessage((msg, connId) => handleMessage(msg, connId));
	setOnConnectionChange((connId, peerState) => {
		if (peerState === "connected") {
			send({
				type: "peer_info",
				data: {
					display_name: state.displayName,
					set_id: state.currentSet,
...
function relayToOthers(msg, fromConnId) {
	for (const [cid] of state.peers) if (cid !== fromConnId) send({
		...msg,
		_relay: true
	}, cid);
}
async function handleMessage(msg, connId) {
	if (!msg || typeof msg !== "object" || typeof msg.type !== "string") return;
	const d = unpackHexFields(msg.data || {});
	if (d && typeof d.ts === "number") {
		const age = Date.now() - d.ts;
		if (age < -12e4 || age > 12e4) return;
	}
	switch (msg.type) {
		case "peer_info": {
			if (!d || typeof d.display_name !== "string" || d.display_name.length > 100 || typeof d.user_id !== "string" || d.user_id.length > 64) return;
			const existing = [...state.peers.values()].find((p) => p.userId === msg.data.user_id);
			state.peers.set(connId, {
				name: msg.data.display_name,
				setId: msg.data.set_id,
			...
			if (state.hostedConnections.has(connId) && msg.data.user_id && !existing) {
				for (const [cid] of state.peers) if (cid !== connId) send({
					type: "new_peer_mesh",
					data: {
						user_id: msg.data.user_id,
						display_name: msg.data.display_name
			...
			if (!state.hostedConnections.has(connId) && !existing) setTimeout(() => sendAll(state.currentSet, connId), 50);
			window._renderPeerList?.();
			break;
		}
		case "new_peer_mesh":
			...
			if (msg.data.user_id !== state.user.id && ![...state.peers.values()].some((p) => p.userId === msg.data.user_id)) {
				const result = await createOffer(state.user.id, state.displayName, state.currentSet);
				send({
					type: "mesh_offer",
					data: {
						offer: result.code,
						from_user: state.user.id,
						from_name: state.displayName,
						for_user: msg.data.user_id,
		...
		case "mesh_offer":
			...
			if (msg.data.for_user === state.user.id || !msg.data.for_user) {
				const { code: answer } = await acceptOffer(msg.data.offer, state.user.id, state.displayName);
				send({
					type: "mesh_answer",
					data: {
						answer,
						to_user: msg.data.from_user,
						offer_connId: msg.data.connId
					}
				}, connId);
			} else {
		...
		case "mesh_answer":
			...
			if (msg.data.to_user === state.user.id || !msg.data.to_user) {
				if (msg.data.offer_connId && hasConnection(msg.data.offer_connId)) await finalizeConnection(msg.data.offer_connId, msg.data.answer);
			} else {
		...
		case "keys": {
			...
			if (!d || typeof d.public_key !== "string" || typeof d.wrapped_dek !== "string" || d.public_key.length > 256 || d.wrapped_dek.length > 512) return;
			...
			if (isPasswordDerived) {
				const existingTeam = await getTeam(sid).catch(() => null);
			...
			await saveTeam({
				team_id: sid,
				name: d.name || sid.slice(0, 8),
				public_key,
				secret_key,
				wrapped_dek: d.wrapped_dek,
				key_derivation: d.key_derivation || "random",
				community_public_key: d.community_public_key || d.public_key,
				community_secret_key: d.community_secret_key || "",
				community_wrapped_dek: d.wrapped_dek || ""
			});
			if (!await getCommunity(sid)) await saveCommunity({
				community_id: sid,
				name: d.name || sid.slice(0, 8),
				description: "",
			...
			if (window._pendingJoinSet) {
				window._pendingJoinSet = false;
		...
		case "new_pin": {
			...
			if (pinData.votes && Array.isArray(pinData.votes) && pinData.votes.length > 0) import("./trust.js").then((mod) => {
			...
			if (sid) await importPin({
				...pinData,
				team_id: sid
			});
			if (sid === state.currentSet) await window._loadPins();
			if (d.author_pubkey && d.author_pubkey !== state.signingPublicKey && sid === state.currentSet) try {
				const dec = decrypt_pin_data(d.ciphertext, d.nonce, state.dek);
				recordNotification({
					type: "pin_added",
					pin_id: d.pin_id,
					pin_title: dec?.title || "Untitled",
					by_pubkey: d.author_pubkey
				});
			} catch (e) {
				console.warn("[sync]", e.message);
			}
		...
		case "delete_pin":
			...
			if (!msg._relay) {
				const existing = await getPin(d.pin_id).catch(() => null);
				if (!existing) {
					console.warn("[sync] delete_pin: pin not found", d.pin_id);
					return;
				}
				if (existing.author_pubkey) {
					...
					if (d.signature) try {
						...
						if (!verify(encode_hex(new TextEncoder().encode(rawPayload)), d.signature, d.by_pubkey)) {
			...
			await deletePin(d.pin_id);
			if (state.currentSet) {
				await window._loadPins();
				window._addHistory?.("Pin deleted (peer)", d.pin_id.slice(0, 8));
			}
			if (!msg._relay) relayToOthers(msg, connId);
			break;
		case "new_drawing": {
			...
			if (sid) await importDrawing({
				...d,
				team_id: sid
			});
			if (sid === state.currentSet) {
				await window._loadDrawings();
				window._addHistory?.("Drawing added (peer)", d.drawing_id.slice(0, 8));
			}
			if (!msg._relay) relayToOthers(msg, connId);
			break;
		}
		case "delete_drawing":
			...
			if (!msg._relay) {
				const existing = await getDrawing(d.drawing_id).catch(() => null);
				if (!existing) {
					console.warn("[sync] delete_drawing: drawing not found", d.drawing_id);
					return;
				}
				if (existing.author_pubkey) {
					if (!d.by_pubkey || existing.author_pubkey !== d.by_pubkey) {
			...
			await deleteDrawing(d.drawing_id);
			if (state.currentSet) {
				await window._loadDrawings();
				window._addHistory?.("Drawing deleted (peer)", d.drawing_id.slice(0, 8));
			}
			if (!msg._relay) relayToOthers(msg, connId);
			break;
		case "sync_pins":
		...
		case "sync_pins_batch": {
			...
			const merged = accumulateBatch(`pins:${d.set_id}`, d.batchIndex, d.totalBatches, d.data);
			if (merged) {
		...
		case "sync_drawings_batch": {
			if (!d || typeof d.set_id !== "string" || !Array.isArray(d.data) || typeof d.batchIndex !== "number" || typeof d.totalBatches !== "number") return;
			const merged = accumulateBatch(`drawings:${d.set_id}`, d.batchIndex, d.totalBatches, d.data);
			if (merged) {
		...
		case "password_required":
			...
			import("./dialogs2.js").then(async (mod) => {
		...
		case "password_verify": {
			...
			const c = await getCommunity(d.set_id);
			if (!c || !c.password_hash) return;
			if (d.password_hash === c.password_hash) {
				passwordAuthPeers.add(connId);
				await sendAll(d.set_id, connId);
			} else broadcast("password_denied", { set_id: d.set_id }, connId);
			break;
		}
		case "password_denied":
			import("./dialogs2.js").then((mod) => mod.toast?.("Wrong password — access denied", "#dc2626"));
			break;
		case "sync_schemas":
			if (!d || typeof d.set_id !== "string" || !Array.isArray(d.data)) return;
			for (const s of d.data || []) if (!(await getSchemas()).find((e) => e.schema_id === s.schema_id)) await saveSchema(s);
			break;
		case "new_annotation": {
			...
			await saveAnnotation({
				...
				created_at: d.created_at || d.ts || Date.now(),
				media: d.media || null,
			...
			if (!msg._relay && d.author_pubkey !== state.signingPublicKey) if (d.parent_id) {
				const parent = await getAnnotation(d.parent_id).catch(() => null);
				if (parent && parent.author_pubkey === state.signingPublicKey) {
					const pin = await getPin(d.pin_id).catch(() => null);
					recordNotification({
						type: "reply",
						pin_id: d.pin_id,
						pin_title: pin?.pin_id ? "Pin" : "Untitled",
						annotation_id: d.annotation_id,
						by_pubkey: d.author_pubkey,
						text_preview: (d.text_preview || "").slice(0, 60)
					});
				}
			} else {
				const pin = await getPin(d.pin_id).catch(() => null);
				if (pin && pin.author_pubkey === state.signingPublicKey) recordNotification({
					type: "comment",
					pin_id: d.pin_id,
					pin_title: "Pin",
					annotation_id: d.annotation_id,
				...
				else if ((await getAnnotationsByPin(d.pin_id, 0, 1)).some((a) => a.author_pubkey === state.signingPublicKey)) recordNotification({
					type: "comment",
					pin_id: d.pin_id,
					pin_title: "Pin",
					annotation_id: d.annotation_id,
		...
		case "annotation_vote": {
			...
			if (!verify(encode_hex(new TextEncoder().encode(rawPayload)), d.signature, d.pubkey)) return;
			const ann = await getAnnotation(d.annotation_id);
			if (!ann) break;
			ann.votes = ann.votes || [];
			const existingIdx = ann.votes.findIndex((v) => v.pubkey === d.pubkey);
			if (existingIdx >= 0) ann.votes[existingIdx] = {
				pubkey: d.pubkey,
			...
			if (!msg._relay && d.pubkey !== state.signingPublicKey && ann.author_pubkey === state.signingPublicKey) recordNotification({
				type: "vote",
				pin_id: ann.pin_id,
				pin_title: "Pin",
				annotation_id: d.annotation_id,
		...
		case "new_tombstone": {
			...
			const existingAnn = await getAnnotation(d.target_id).catch(() => null);
			...
			if (d.signature && verify && encode_hex) try {
			...
			await saveTombstone({
				tombstone_id: d.tombstone_id,
				target_id: d.target_id,
				by_pubkey: d.by_pubkey,
				reason: d.reason || "",
				timestamp: d.timestamp || d.ts || Date.now(),
				signature: d.signature || ""
			});
			const ann = await getAnnotation(d.target_id);
		...
		case "sync_annotations_batch": {
			if (!d || typeof d.set_id !== "string" || !Array.isArray(d.data) || typeof d.batchIndex !== "number" || typeof d.totalBatches !== "number") return;
			const merged = accumulateBatch(`annotations:${d.set_id}`, d.batchIndex, d.totalBatches, d.data);
			if (merged) await processSyncAnnotations(d.set_id, merged);
			break;
		}
		case "sync_request":
			await sendAll();
		...
		case "map_view": {
			...
			} else if (d.center && state.map) window._pendingMapView = {
				center: d.center,
				zoom: d.zoom || 5
			};
			const peer = state.peers.get(connId);
			if (d.center && peer && d.team_id) {
				window._peerLocations.set(connId, {
					lat: d.center[0],
					lng: d.center[1],
					name: peer.name,
					team_id: d.team_id
		...
		case "schema_publish": {
			...
			const existing = await getSchemas();
			if (!existing.find((e) => e.schema_id === d.schema_id)) await saveSchema({
			...
			else if (d.version && existing.find((e) => e.schema_id === d.schema_id && (e.version || 1) < d.version)) await saveSchema({
				...existing.find((e) => e.schema_id === d.schema_id),
				name: d.name,
				fields: d.fields,
				version: d.version,
		...
		case "sync_chains_batch": {
			if (!d || typeof d.set_id !== "string" || !Array.isArray(d.data) || typeof d.batchIndex !== "number" || typeof d.totalBatches !== "number") return;
			const merged = accumulateBatch(`chains:${d.set_id}`, d.batchIndex, d.totalBatches, d.data);
			if (merged) processSyncChains(d.set_id, merged);
			break;
		}
	}
}
function broadcastPinVote(pinId, dir) {
	if (!state.currentSet) return;
	import("./relay.js").then((r) => r.sendPinVote(state.currentSet, pinId, dir)).catch((e) => {
		console.warn("[sync] pin vote relay failed:", e.message);
	});
}
async function processSyncPins(setId, data) {
	const sid = setId || window._pendingSet;
	await importPins((data || []).map((p) => {
...
async function processSyncDrawings(setId, data) {
	const sid = setId || window._pendingSet;
	await importDrawings((data || []).map((d) => ({
		...d,
		team_id: sid
	})));
...
async function processSyncAnnotations(setId, data) {
	await saveAnnotations((data || []).map((a) => ({
		...a,
		community_id: setId
	})));
...
async function processSyncChains(setId, data) {
	for (const c of data || []) await saveChain({
		...c,
		community_id: setId || c.community_id
	});
	window._loadChains?.();
}
async function sendAll(sid, connId) {
	const set = sid || state.currentSet;
	if (!isConnected() || !set) return;
	const t = await getTeam(set);
	const c = await getCommunity(set);
	if (t && connId && c?.password_hash && !passwordAuthPeers.has(connId)) {
		broadcast("password_required", {
			set_id: set,
	...
	const BATCH_SIZE = 50;
	const allPins = await getAllPins(set);
	if (allPins && allPins.length > 0) for (let i = 0; i < allPins.length; i += BATCH_SIZE) broadcast("sync_pins_batch", {
		set_id: set,
		batchIndex: Math.floor(i / BATCH_SIZE),
		totalBatches: Math.ceil(allPins.length / BATCH_SIZE),
		data: allPins.slice(i, i + BATCH_SIZE)
	}, connId);
	else broadcast("sync_pins", {
		set_id: set,
		data: []
	}, connId);
	const allDrawings = await getAllDrawings(set);
	if (allDrawings && allDrawings.length > 0) for (let i = 0; i < allDrawings.length; i += BATCH_SIZE) broadcast("sync_drawings_batch", {
		set_id: set,
		batchIndex: Math.floor(i / BATCH_SIZE),
		totalBatches: Math.ceil(allDrawings.length / BATCH_SIZE),
		data: allDrawings.slice(i, i + BATCH_SIZE)
	}, connId);
	else broadcast("sync_drawings", {
		set_id: set,
		data: []
	}, connId);
	const anns = await getAnnotationsByCommunity(set);
	if (anns && anns.length > 0) for (let i = 0; i < anns.length; i += BATCH_SIZE) broadcast("sync_annotations_batch", {
		set_id: set,
		batchIndex: Math.floor(i / BATCH_SIZE),
		totalBatches: Math.ceil(anns.length / BATCH_SIZE),
		data: anns.slice(i, i + BATCH_SIZE)
	}, connId);
	const chains = await getChainsByCommunity(set);
	if (chains && chains.length > 0) for (let i = 0; i < chains.length; i += BATCH_SIZE) broadcast("sync_chains_batch", {
		set_id: set,
		batchIndex: Math.floor(i / BATCH_SIZE),
		totalBatches: Math.ceil(chains.length / BATCH_SIZE),
		data: chains.slice(i, i + BATCH_SIZE)
	}, connId);
	else broadcast("sync_chains", {
		set_id: set,
		data: []
	}, connId);
	const schemas = await getSchemas();
	if (schemas && schemas.length) broadcast("sync_schemas", {
		set_id: set,
		data: schemas
	}, connId);
	if (state.map) {
		const s = await getSettings(set);
		broadcast("map_view", {
			center: s?.map_center || [state.map.getCenter().lat, state.map.getCenter().lng],
			zoom: s?.map_zoom || state.map.getZoom()
		}, connId);
	}
}
function broadcast(type, data, connId) {
	if (!state.currentSet) return;
	let meshData = data;
	if (state.signingPublicKey) {
		if (type === "delete_pin" || type === "delete_drawing") {
			meshData = {
				...data,
				by_pubkey: state.signingPublicKey
			};
			if (state.signingSecretKey) try {
				const ts = Date.now();
				const rawPayload = (data.pin_id || data.drawing_id) + "|" + ts;
				const sig = sign(encode_hex(new TextEncoder().encode(rawPayload)), state.signingSecretKey);
				if (sig) meshData = {
					...meshData,
					signature: sig,
					timestamp: ts
	...
	if (type === "new_pin") window._relayPushDelta?.(state.currentSet, [{
		...data,
		team_id: void 0,
		set_id: void 0,
		ts: void 0
	}], [], [], [], [], []);
	else if (type === "new_drawing") window._relayPushDelta?.(state.currentSet, [], [], [{
		...data,
		drawing_id: data.drawing_id
	}], [], [], []);
	else if (type === "new_annotation") window._relayPushDelta?.(state.currentSet, [], [{
		...data,
		annotation_id: data.annotation_id
	}], [], [], [], []);
	else if (type === "new_tombstone") window._relayPushDelta?.(state.currentSet, [], [], [], [{
		...data,
		tombstone_id: data.tombstone_id
	}], [], []);
	else if (type === "delete_pin") window._relayPushDelta?.(state.currentSet, [], [], [], [], [data.pin_id], []);
	else if (type === "delete_drawing") window._relayPushDelta?.(state.currentSet, [], [], [], [], [], [data.drawing_id]);
	else if (type === "new_chain") window._relayPushDelta?.(state.currentSet, [], [], [], [], [], [], [data], []);
	else if (type === "delete_chain") window._relayPushDelta?.(state.currentSet, [], [], [], [], [], [], [], [data.chain_id]);
	const payload = packHexFields({
		...data,
		team_id: state.currentSet,
		ts: Date.now()
	});
	const chunks = splitMessage(JSON.stringify({
		type,
		data: payload
	}));
	if (!chunks) send({
	...
	else for (let i = 0; i < chunks.total; i++) send({
		type: "chunk",
		data: {
			id: chunks.id,
			index: i,
...
async function hostGroup() {
	...
	try {
		const { connId, code, compact } = await createOffer(state.user.id, state.displayName, state.currentSet);
		...
		ov.remove();
		await alertDialog("Failed: " + e.message);
	}
}
function connectRelayRoom({ relayUrl, roomId, password, title, retryFn, onWelcome, onMessage }) {
	...
	ws.onerror = () => {
		...
		document.getElementById("relay-retry").onclick = () => {
			done = true;
			ws.close();
			ov.remove();
			retryFn();
		};
		document.getElementById("relay-close").onclick = () => {
			done = true;
			ws.close();
			ov.remove();
		};
...
async function hostGroupViaRelay(relayUrl) {
	if (!state.currentSet) return;
	...
	const createOfferFor = async (targetId) => {
		try {
			const { connId, code } = await createOffer(state.user.id, state.displayName, state.currentSet);
			state.hostedConnections.add(connId);
			...
			toast("Failed to connect to peer — retry", "#dc2626");
		}
	};
	let ws;
	connectRelayRoom({
		...
		onMessage: async (msg, w, clientId) => {
			ws = w;
			if (msg.type === "peer_joined") createOfferFor(msg.clientId);
			...
			if (msg.type === "answer") {
				...
				if (connId) {
					await finalizeConnection(connId, msg.code);
					hostPeerHandshake(msg.display_name || "Peer", connId);
					pendingOffers.delete(connId);
					window._renderUI?.();
				} else toast("Received answer for unknown peer", "#f97316");
			}
		}
	});
}
async function joinPeerViaRelay(relayUrl, roomId) {
	connectRelayRoom({
		relayUrl,
		roomId,
		password: await promptRoomPassword("Room password (leave blank if none)"),
		...
		onMessage: async (msg, ws, clientId, cleanup) => {
			if (msg.type === "offer" && msg.code) {
				const { setId, connId, code: answer } = await acceptOffer(msg.code, state.user.id, state.displayName);
				ws.send(JSON.stringify({
					type: "answer",
					code: answer,
					connId,
...
async function exportSet() {
	...
	showPasswordDialog("Export — set a password (or leave blank):", async (password, compressVideos) => {
		...
		try {
			...
			const t = await getTeam(state.currentSet);
			const s = await getSettings(state.currentSet);
			const c = await getCommunity(state.currentSet);
			const pins = await getAllPins(state.currentSet);
			const drawings = await getAllDrawings(state.currentSet);
			...
			const data = await compactStoredMedia({
				...
				keys: t ? {
					...
					secret_key: t.secret_key,
					wrapped_dek: t.wrapped_dek,
					key_derivation: t.key_derivation || "random",
					community_public_key: t.community_public_key || "",
					community_secret_key: t.community_secret_key || "",
					community_wrapped_dek: t.community_wrapped_dek || ""
				} : null,
			...
			if (password) {
				...
				payload = new Uint8Array(29 + ct.length);
				payload[0] = 1;
				payload.set(salt, 1);
				payload.set(nonce, 17);
				payload.set(ct, 29);
			} else payload = new TextEncoder().encode(json);
			...
			a.href = url;
			a.download = "🗺️ " + (window._names[state.currentSet] || state.currentSet || "export") + ".piggpin";
...
async function shareMap() {
	...
	showPasswordDialog("Share — set a password (or leave blank):", async (password) => {
		...
		try {
			...
			showShareMethodDialog(compressed, compressed.length > 200 * 1024, bgm, {
				pinCount,
				dwgCount,
				mediaCount,
				rawSize
...
function generateCommunityLinkUrl(community, communitySk) {
	...
	const flags = (community.password_hash ? 1 : 0) | (communitySk ? 4 : 0);
...
function showShareMethodDialog(compressed, tooLarge, bgm, preview = {}, jsonPayload = null, community = null) {
	...
	const copy = async (url, msg) => {
		...
		toast(msg || "Share link copied to clipboard", "#16a34a");
	};
	document.getElementById("sm-embed").onclick = () => {
		clean();
		const urlCode = base64url_encode(jsonPayload || compressed);
		copy(`<iframe src="${window.location.origin + window.location.pathname + "?embed=1#map=" + urlCode}" width="100%" height="400" frameborder="0" allowfullscreen></iframe>`, "Embed code copied to clipboard");
	};
	...
	document.getElementById("sm-tinyurl").onclick = async () => {
		...
		try {
			...
			if (resp.ok) {
				...
				if (t && t.startsWith("http")) short = t;
			}
		} catch (e) {
			console.warn("[sync]", e.message);
		}
		if (short) copy(short);
			...
			copy(longUrl);
			toast("TinyURL failed — raw URL copied", "#f97316");
		}
	};
	if (relayUrl) document.getElementById("sm-relay").onclick = async () => {
		...
		let resp = null;
		try {
			for (const u of [httpUrl, altUrl]) try {
				const r = await fetch(u + "/share" + qs, {
					method: "POST",
	...
	if (hasExistingCommunity && isCommunityPublished) document.getElementById("sm-community").onclick = async () => {
		const team = await getTeam(community.community_id);
		const url = generateCommunityLinkUrl(community, team?.community_secret_key || team?.secret_key || "");
		if (url) {
			...
			import("./e2e_core.js").then((mod) => {
				...
				document.getElementById("sm-community").parentElement.appendChild(qrWrap);
			}).catch(() => {});
		}
	};
	if (hasExistingCommunity && !isCommunityPublished) document.getElementById("sm-community-setup").onclick = () => {
		const cid = community.community_id;
		import("./map.js").then((m) => m.showCommunityDetails(cid)).catch(() => {});
	};
	if (!hasExistingCommunity) document.getElementById("sm-community-create").onclick = async () => {
...
async function importSet() {
	...
	fileInput.type = "file";
	fileInput.accept = ".piggpin,.txt,.geojson,.json,.kml,.gpx,.csv";
	fileInput.onchange = async () => {
		...
		if (ext === "kml") {
			if (text.includes("<kml") || text.includes("<Placemark")) {
				await importKML(text);
				return;
			}
		}
		if (ext === "gpx") {
			if (text.includes("<gpx") || text.includes("<wpt") || text.includes("<trk")) {
				await importGPX(text);
				return;
			}
		}
		if (ext === "csv") {
		...
		if (bytes.length >= 2 && bytes[0] === 31 && bytes[1] === 139) {
			let data;
			try {
				data = decompress_gzip(bytes);
			} catch (_) {
				await alertDialog("Invalid data");
				return;
			}
			...
			if (data[0] === 1) {
				...
				showPasswordDialog("Import — enter password:", async (password) => {
					try {
						...
						console.error("[import] password-protected .piggpin:", e);
						await alertDialog("Wrong password or invalid data");
					}
				});
			} else try {
				await doImport(unpackHexFields(JSON.parse(new TextDecoder().decode(data))));
			} catch (e) {
				console.error("[import] plain .piggpin:", e);
				await alertDialog("Invalid data");
			}
		} else {
			const text = new TextDecoder().decode(bytes);
			if (text.startsWith("ENCRYPTED:")) showPasswordDialog("Import — enter password:", async (password) => {
				try {
					const parts = text.split(":");
					const dec = decrypt_with_password(parts[1], parts[2], parts[3], password);
					await doImport(unpackHexFields(JSON.parse(dec)));
				} catch (e) {
					console.error("[import] ENCRYPTED text format:", e);
					await alertDialog("Wrong password or invalid data");
				}
			});
			else try {
...
async function importFromHash(urlCode) {
	try {
		...
		if (data[0] === 1) {
			...
			return new Promise((resolve) => {
				showPasswordDialog("Share link — enter password:", async (password) => {
					...
					try {
						await doImport(deserializeBinary(decrypt_bytes_with_password(encode_hex(ct), encode_hex(nonce), encode_hex(salt), password)));
						resolve(true);
					} catch (_) {
						resolve(false);
					}
...
async function importFromCompressed(compressed) {
	try {
...
async function doImport(data) {
	...
	if (data.keys) {
		...
		await saveTeam({
			...
			community_wrapped_dek: community_wrapped_dek || ""
		});
	}
	await saveCommunity({
		community_id: sid,
	...
	await importPins((data.pins || []).map((p) => ({
		...p,
		team_id: sid
	})));
	await importDrawings((data.drawings || []).map((d) => ({
		...d,
		team_id: sid
	})));
	if (data.schemas && Array.isArray(data.schemas)) {
		const existing = await getSchemas();
		for (const s of data.schemas) if (!existing.find((e) => e.schema_id === s.schema_id)) await saveSchema({
			schema_id: s.schema_id,
			name: s.name,
			fields: s.fields || []
		});
	}
	if (data.layers && Array.isArray(data.layers) && data.layers.length > 0) await saveLayers(sid, data.layers);
	else await saveLayers(sid, [{
	...
	if (data.chains && Array.isArray(data.chains)) for (const c of data.chains) await saveChain({
		...c,
		community_id: sid,
		chain_id: generate_uuid()
	});
	window._names[sid] = data.name || "Imported";
	if (data.map_center) await saveSettings(sid, {
...
function getExportSummary() {
	return {
		pinCount: state.markers.filter((m) => m._pinId).length,
		drawingCount: (window._drawingLayers || []).length
	};
}
...
function geojsonToKmlGeometry(geom) {
	...
	if (geom.type === "Point") return `<Point><coordinates>${coords[0]},${coords[1]},0</coordinates></Point>`;
	if (geom.type === "LineString") return `<LineString><coordinates>${coords.map((c) => `${c[0]},${c[1]},0`).join(" ")}</coordinates></LineString>`;
	...
	if (geom.type === "MultiLineString") return coords.map((ring) => `<LineString><coordinates>${ring.map((c) => `${c[0]},${c[1]},0`).join(" ")}</coordinates></LineString>`).join("");
	if (geom.type === "MultiPolygon") return coords.map((poly) => `<Polygon><outerBoundaryIs><LinearRing><coordinates>${poly[0].map((c) => `${c[0]},${c[1]},0`).join(" ")}</coordinates></LinearRing></outerBoundaryIs></Polygon>`).join("");
	return "";
}
async function exportGeoJSON() {
	if (!state.currentSet || !state.dek) return;
	getExportSummary();
	if (!await confirmDialog(`Exporting as GeoJSON will exclude media, schema data, and attestations. This format is best for sharing coordinates with other mapping tools.\n\nContinue?`)) return;
	...
	try {
		...
		for (const row of drawings) {
			try {
				...
				if (!geo) continue;
				const layer = state.layers.find((l) => l.layer_id === row.layer_id);
				const feature = geo.type === "Feature" ? geo : {
					type: "Feature",
					geometry: geo
				};
				feature.properties = {
		...
		a.href = url;
		a.download = name.replace(/[^a-zA-Z0-9 _-]/g, "_") + ".geojson";
		document.body.appendChild(a);
		a.click();
		document.body.removeChild(a);
		URL.revokeObjectURL(url);
...
async function exportKML() {
	if (!state.currentSet || !state.dek) return;
	if (!await confirmDialog("Exporting as KML will exclude media, schema data, and attestations. This format is compatible with Google Earth and many GIS tools.\n\nContinue?")) return;
...
<Document>
  <name>${escapeXml(window._names[state.currentSet] || state.currentSet || "export")}</name>
`;
		let total = pins.length + drawings.length;
		let done = 0;
		for (const row of pins) {
...
async function exportGPX() {
	if (!state.currentSet || !state.dek) return;
	if (!await confirmDialog("Exporting as GPX will exclude media, schema data, and attestations. This format is compatible with GPS devices, OsmAnd, and Organic Maps.\n\nContinue?")) return;
...
<metadata><name>${escapeXml(window._names[state.currentSet] || state.currentSet || "export")}</name></metadata>
`;
		let total = pins.length + drawings.length;
		let done = 0;
		for (const row of pins) {
...
`;
		...
		for (const row of drawings) {
			try {
				...
				const geom = geo.type === "Feature" ? geo.geometry : geo;
				const title = geo.properties?.title || "Drawing";
				if (geom.type === "LineString" && geom.coordinates.length >= 2) gpx += `  <trk>
    <name>${escapeXml(title)}</name>
    <trkseg>
...
`;
				else if (geom.type === "Point") gpx += `  <wpt lat="${geom.coordinates[1]}" lon="${geom.coordinates[0]}">
    <name>${escapeXml(title)}</name>
  </wpt>
`;
			} catch (e) {
...
async function importGeoJSON(text) {
	...
	try {
		...
		await alertDialog("Invalid GeoJSON file");
		return;
	}
	const features = data.type === "FeatureCollection" ? data.features : data.type === "Feature" ? [data] : Array.isArray(data) ? data : [];
	if (!features.length) {
		await alertDialog("No features found in GeoJSON file");
		return;
	}
	if (!await confirmDialog(`Import ${features.length} feature(s) into the current map? They will be encrypted with this map's key.`)) return;
	...
	for (let i = 0; i < features.length; i++) {
		const f = features[i];
		const geom = f.geometry || (f.type === "Point" ? {
			type: "Point",
			coordinates: [f.lon || f.lng, f.lat]
		} : null);
		if (!geom) {
			skipped++;
			continue;
		...
		try {
			if (geom.type === "Point" && geom.coordinates) {
				...
				await saveDrawing({
					...
					geometry: geom,
					properties: {
						title: props.title || "Imported Drawing",
						note: props.note || "",
...
async function importCSV(text) {
	...
	if (lines.length < 2) {
		await alertDialog("CSV must have a header row and at least one data row");
		return;
	}
	const headers = lines[0].split(",").map((h) => h.trim().toLowerCase());
	const latIdx = headers.indexOf("lat"), lngIdx = headers.indexOf("lng");
	if (latIdx < 0 || lngIdx < 0) {
		await alertDialog("CSV must have 'lat' and 'lng' columns");
		return;
	}
	...
	if (!await confirmDialog(`Import ${dataRows.length} row(s) into the current map? They will be encrypted with this map's key.`)) return;
...
async function importKML(text) {
	...
	if (doc.querySelector("parsererror")) {
		await alertDialog("Invalid KML file");
		return;
	}
	...
	if (!toParse.length) {
		await alertDialog("No Placemarks found in KML file");
		return;
	}
	if (!await confirmDialog(`Import ${toParse.length} placemark(s) into the current map? They will be encrypted with this map's key.`)) return;
	...
	for (let i = 0; i < toParse.length; i++) {
		...
		try {
			if (pointEl?.[0]) {
				...
				if (coordEl?.[0]) {
					const coords = coordEl[0].textContent.trim().split(/\s+/).map((p) => {
						const a = p.split(",");
...
async function importGPX(text) {
	...
	const doc = new DOMParser().parseFromString(text, "text/xml");
	if (doc.querySelector("parsererror")) {
		await alertDialog("Invalid GPX file");
		return;
	}
	const getTag = (el, tag) => {
		const ns = el.getElementsByTagNameNS?.("*", tag);
	...
	if (!total) {
		await alertDialog("No waypoints or tracks found in GPX file");
		return;
	}
	if (!await confirmDialog(`Import ${wpts.length} waypoint(s) and ${trksegs.length} track(s) into the current map? They will be encrypted with this map's key.`)) return;
	...
	for (const seg of trksegs) {
		...
		if (coords.length >= 2) {
			...
			const title = (trk ? getTag(trk, "name") : null)?.[0]?.textContent || "Track";
			try {
				const { saveDrawing } = await import("./map.js");
				await saveDrawing({
					type: "Feature",
...
function showExportFormatModal() {
	...
	document.getElementById("exp-fmt-piggpin").onclick = () => {
		clean();
		exportSet();
	};
	document.getElementById("exp-fmt-geojson").onclick = () => {
		clean();
		exportGeoJSON();
	};
	document.getElementById("exp-fmt-kml").onclick = () => {
		clean();
		exportKML();
	};
	document.getElementById("exp-fmt-gpx").onclick = () => {
		clean();
		exportGPX();
	};
}
async function rotateSetKeys() {
	if (!state.currentSet || !state.dek) return;
	...
	for (const row of pins) try {
		...
		row.nonce = enc.nonce;
		if (row.media) {
			const menc = encrypt_raw_bytes(decrypt_raw_bytes(row.media.ciphertext, row.media.nonce, oldDek), newDek);
			row.media.ciphertext = menc.ciphertext;
			row.media.nonce = menc.nonce;
		}
		await savePin(row);
	} catch (_) {
		pinErrors++;
	...
	for (const row of drawings) try {
		const enc = encrypt_geojson(decrypt_geojson(row.encrypted_geojson, row.nonce, oldDek), newDek);
...
function broadcastAnnotation(annotation) {
	...
	if (state.currentSet) _meshBroadcast?.("new_annotation", {
		annotation_id: annotation.annotation_id,
		pin_id: annotation.pin_id,
		ciphertext: annotation.ciphertext,
		nonce: annotation.nonce,
...
function broadcastAnnotationVote(annotationId, vote) {
...
function broadcastTombstone(tombstone) {
...
//#endregion
export { setMeshBroadcast as C, unpackHexFields as D, showExportFormatModal as E, sendAll as S, shareMap as T, importGeoJSON as _, broadcastTombstone as a, joinPeerViaRelay as b, exportGeoJSON as c, handleMessage as d, hostGroup as f, importFromHash as g, importFromCompressed as h, broadcastPinVote as i, exportKML as l, importCSV as m, broadcastAnnotation as n, doImport as o, hostGroupViaRelay as p, broadcastAnnotationVote as r, exportGPX as s, broadcast as t, exportSet as u, importSet as v, setupPeer as w, rotateSetKeys as x, joinPeer as y };
