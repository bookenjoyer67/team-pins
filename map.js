export function addPickMarker(lat, lng) {
  ...
  marker.on('dragend', () => {
    ...
    try {
      window.parent.postMessage({ type: 'piggpin:location-picked', lat: pos.lat, lng: pos.lng }, '*');
    } catch (_) {}
  });
  try {
    console.log('[piggpin] pick: sending initial coords', lat, lng);
    window.parent.postMessage({ type: 'piggpin:location-picked', lat, lng }, '*');
  } catch (_) {}
  if (pickPinData) {
    window._broadcast?.("new_pin", { ...pickPinData, team_id: state.currentSet });
